...
Sub1<-IP%>%mutate(deaths=`Deaths A`, force=SideAPeakTotForces, forcetheat=`SideAPeak TheatForces`)%>%select(WarNum,deaths,WDuratDays,Americas,WarTypeD,Initiator,SideA,force,forcetheat)
Sub1$side='A'
Sub1$Init=ifelse(Sub1$SideA==Sub1$Initiator,1,0)
SubA<-Sub1%>%select(-c(Initiator,SideA))

Sub2<-IP%>%mutate(deaths=`Deaths B`, force=SideBPeakTotForces, forcetheat=`SideBPeakTheatForces`)%>%select(WarNum,deaths,WDuratDays,Americas,WarTypeD,Initiator,SideB,force,forcetheat)
Sub1$side='A'
Sub2$side='B'
